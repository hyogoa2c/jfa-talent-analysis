...
**規則（推測しない）**:
- **クラブユース・高校の後に大学へ進んでからプロ入りした場合は `university`。**
  アカデミー出身であっても、プロ入り直前が大学ならそれが答えである。
  例: 「浦和レッズユース→**法政大学**→カターレ富山」は `university` / 法政大学。
  「京都パープルサンガユース→**関西大学**→京都サンガ」は `university` / 関西大学。
  経歴に大学が現れたら、それが**プロ入りの前か後か**を必ず確かめる
  （プロ入り**後**に進学した大学は答えではない。例:「磐田ユース→磐田→FC岐阜→中京大学」は
  `j_club_academy`）。
